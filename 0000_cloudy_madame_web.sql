CREATE TABLE `company` (
	...
	`internal__last_website_sync` integer
);
...
CREATE TABLE `user_to_company` (
	...
	`company_gecko_id` text NOT NULL,
	PRIMARY KEY(`user_gecko_id`, `company_gecko_id`),
...
CREATE TABLE `user_to_event` (
	...
	`gecko_event_participant` text,
	`gecko_global_id` text,
	`last_seen_gecko` integer NOT NULL,
	`last_seen_website` integer NOT NULL,
	PRIMARY KEY(`user_gecko_id`, `event_gecko_id`),
	FOREIGN KEY (`user_gecko_id`) REFERENCES `user`(`user_id`) ON UPDATE cascade ON DELETE cascade,
...
CREATE UNIQUE INDEX `event_gecko_global_id_unique` ON `event` (`gecko_global_id`);--> statement-breakpoint
CREATE UNIQUE INDEX `product_uid_unique` ON `product` (`uid`);--> statement-breakpoint
CREATE UNIQUE INDEX `user_gecko_global_id_unique` ON `user` (`gecko_global_id`);